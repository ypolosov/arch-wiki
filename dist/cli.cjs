...
function migrationContext(opts) {
  ...
  return {
    abs: (relPath) => path5.join(root, relPath),
    fs: fs2,
...
function templatesDir() {
  return process.env.ARCH_WIKI_TEMPLATES_DIR ?? path5.join(pluginRoot(), "templates");
}
function wikiRoot(opts) {
  const cwd = opts.cwd ?? process.cwd();
  const root = opts.root ?? "docs/architecture";
  return path5.isAbsolute(root) ? root : path5.join(cwd, root);
}
function emit(env) {
